...
###
TimePoints <- c("Seq1", "Seq2 ")		## how many points in time does the experiment cover? Provide a label for each time-point in the format c("T0", "T1", "T2", ...) or leave at 'FALSE' for no time-splitting
...
###
columnNamesL1 <- c("C_water")			## please note that there is a special prefix for class- and numerical variables: the default is "C_" for class-variables and "Y_" for numerical variables
...
L1  <- list(list("StU", "Ob", "MQs")) 	## Please look at the vignette 'Examples for Experiment-Design' for more information on the use of the arguments L1 and L2.
# L2  <- list(list("")) 
L2 <- L1
###
