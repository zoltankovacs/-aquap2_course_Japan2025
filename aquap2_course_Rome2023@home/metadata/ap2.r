...
##################### split dataset ############################
################################################################
spl.var <- c("C_waterNames", "C_Group")							## by which variables should the dataset be split? leave empty c() or set to NULL for *no* splitting by class variables, or provide one or more valid class variables to split along these variables; e.g. "C_Group", or c("C_Group", "C_Mode", ...)
...
#########################
### PCA 
do.pca <- TRUE							## if PCA of the given datasets should be performed

# plotting 
pca.colorBy <- c("C_conSNr")						## which variables to use for coloring the score plots. leave empty c() or set to NULL for using all available class-variables for coloring. Provide a character vector with valid class variable names to color along these variables; e.g. c("C_Group", "C_Temp", "C_RelHum", ...)
...
#########################
### PLSR
do.pls <- TRUE							## if PLSR models of the given datasets should be calculated
pls.ncomp <- NULL						## number of components, leave at NULL for automatic detection of optimal number of components
pls.regOn <- c("Y_conSNr", "Y_RelHum", "Y_absTime")				## which variables should be used to regress on? Leave empty c() or set to NULL for using all numerical variables, or provide a character vector with valid numerical variable names to regress on these variables; e.g. "Y_Temp", or c("Y_Temp", "Y_FooBar", ...)
pls.valid <- c("C_conSNr", "C_RelHum", "C_conSNr")				## 'def' to get the default value from the settings file (parameter 'plsr_calc_typeOfCrossvalid'); a number to perform this n-fold crossvalidation, 'LOO' for a leave-one-out crossvalidation, or a valid name of a class variable for performing a crossvalidation based on the grouping defined by this variable. Can be of equal length as the vector in 'pls.regOn' to perform the corresponding type of CV for each element in 'pls.regOn'.
...
# plotting
# pls.colorBy <- "C_conSNr"				## What class-variable should be used for coloring in the RMSEP and RMSECV plots? Leave empty c() or set to NULL for no coloring, or provide a character length one with a valid class variable name to color along this variable; e.g. "C_Group"
pls.colorBy <- "C_RelHum"
