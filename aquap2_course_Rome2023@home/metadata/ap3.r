...
spl.wl <- "1300-to-1600"				## which wavelengths to use? leave empty c() or set to NULL for using all available wavelengths. Provide a character vector in the format e.g. c("1300-to-1600"), or c("1000-to-2000", "1300-to-1600", ...) to split along these wavelengths.
#######
...
#######
spl.do.csAvg <- TRUE					## if all the consecutive scans of a single sample should be reduced, i.e. averaged into a single spectrum
...
# plotting 
pca.colorBy <- c("C_waterNames", "C_conSNr")	## which variables to use for coloring the score plots. leave empty c() or set to NULL for using all available class-variables for coloring. Provide a character vector with valid class variable names to color along these variables; e.g. c("C_Group", "C_Temp", "C_RelHum", ...)
...
pca.elcolorBy <- NULL  #"C_RelHum"					## which variables to use for plotting additional confidence intervall ellipses. Set to NULL for not drawing additional CI-ellipses. Provide one variable (gets recycled) or a vector with equal length as pca.colorBy to have the additional ci-ellipses along these variables; e.g. "C_RelHum"
...
pca.sc <- c(2, 3)						## Numeric length 2, the two PCs to be plotted against each other in the score plots.
...
pca.lo <- 1:4							## Numeric sequence of length >=2, indicating what PC components to plot in the loadingplot.
#########################
...
sim.vars <- c("C_waterNames", "C_conSNr")	## which variables should be use to group the data? Leave empty c() or set to NULL for using all available class-variables, or provide a character vector with valid class variable names to group along these variables; e.g. "C_Group", or c("C_Group", "C_FooBar", ...)
sim.K <- 2								## The number of components. In mode "robust", leave at 0 for automatic detection. (it is a capital "K")
...
#########################
### Aquagram
do.aqg <- TRUE							## if Aquagrams of the given datasets should be calculated
aqg.vars <- c("C_waterNames")			## which variables should be use to group the data? Provide a character vector with valid class variable names to group along these variables; e.g. "C_Group", or c("C_Group", "C_FooBar", ...)
